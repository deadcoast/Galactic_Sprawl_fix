...
## CURRENT SCRATCHPAD 2025-06-06

- [ ] ShipFactory type conformance fixes (TS2322)
  - [ ] Ensure `factionStats?.formation` cast/transform matches `CombatShip['formation']` shape
  - [ ] Ensure `factionStats?.combatStats` matches typed structure `{ damageDealt; damageReceived; killCount; assistCount }`
- [ ] WebGLShaderManager WebGL API typo (TS2339)
  - [ ] Replace `gl.dracombatrays` with valid extension/placeholder guard
- [ ] CombatShipManager remaining unsafe assignments (lines 165-184 & 312-324)
  - [ ] Refactor `ship.combatStats` initialization to a safe builder/helper
  - [ ] Remove residual `console` statements (no-console)
  - [ ] Address Sourcery nested-if suggestion if applicable
- [ ] ExplorationManager abstract contract mismatches (TS2345, TS2416, TS2352)
  - [ ] Update `onInitialize`, `onDispose` to async Promise signatures
  - [ ] Correct subscribe callback typings for `ExplorationEvent`
- [ ] ResourceManager compilation failure block (multiple TS errors around onUpdate)
  - [ ] Rewrite broken method starting at line 295 to proper `protected onUpdate(deltaTime:number)`
  - [ ] Add async `onInitialize`/`onDispose` Promise signatures
- [ ] MiningResourceIntegration & MiningShipManager math/type errors
  - [ ] Guard undefined numeric fields before arithmetic/comparison
  - [ ] Fix incorrect `stats.capabilities` property reference (should be abilities or update type)
- [ ] Module Managers (`ModuleAttachmentManager`, `ModuleStatusManager`, `ModuleUpgradeManager`) missing `clearAllListeners`
  - [ ] Implement interface in base or add stub methods
- [ ] ShipHangarManager category type mismatch (TS2322)
  - [ ] Map `PlayerShipCategory` to `ShipCategory` enum or extend union
- [ ] ResourceFlowManager optional chaining safety (TS18048)
  - [ ] Add nullish-coalescing default for `capacity`
- [ ] WeaponUpgradeManager subscribe signature mismatch (TS2416)
  - [ ] Align `subscribe` return type with base `TypedEventEmitter`
- [ ] Logging services typo (`logwarn`) in Data* & EventPropagationService (TS2551)
  - [ ] Replace with `logWarn` and add tests
- [ ] Worker.ts ErrorType enum usage & unsafe access
  - [ ] Add `WORKER` member to `ErrorType` or change reference
  - [ ] Eliminate all unsafe any/member-access, remove `console` logs
- [ ] ThemeTypes palette type mismatches (TS2322, TS2345)
  - [ ] Refactor palette merge utility to explicit mapped type
- [ ] UI/EventTypes generic predicate issues (TS2707, TS2677)
  - [ ] Provide generic param to `TypedEvent` and fix predicate return type
- [ ] Tests & Performance suites unknown property / unknown type errors
  - [ ] Add proper Jest expect matchers or cast to `unknown as ...`
- [ ] Missing modules in MockManager & SharedManager types
  - [ ] Create `SharedEventTypes` barrel or adjust imports
