...
async function main() {
  ...
  try {
    ...
    for (const file of plan.relocatableFiles) {
      console.log(`   • ${file.path} → ${file.proposedNewPath} (${file.category})`);
      if (file.toolsAffected.length > 0) {
        console.log(`     Tools: ${file.toolsAffected.join(', ')}`);
      }
    }
    ...
    console.log(`   • During-move tests: ${plan.validationSuite.duringMove.length}`);
    console.log(`   • Post-move tests: ${plan.validationSuite.postMove.length}`);
