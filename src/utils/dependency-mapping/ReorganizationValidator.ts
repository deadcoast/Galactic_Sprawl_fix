...
export class ReorganizationValidator {
  ...
  public async runCompleteAnalysis(): Promise<ReorganizationPlan> {
    ...
    const toolsValidation = await this.baselineValidator.validateToolsAvailability();
  ...
  private generateRecommendations(
    ...
    if (!buildValidation.isValid) {
      recommendations.push('Fix build system errors before proceeding with reorganization');
      warnings.push(...buildValidation.errors);
    }

    // Tool availability recommendations
    if (!toolsValidation.allAvailable) {
      recommendations.push('Install missing development tools before reorganization');
      warnings.push(...toolsValidation.missingTools.map((tool: string) => `Missing tool: ${tool}`));
    }

    // Baseline test recommendations
    if (!baselineReport.summary.allPassed) {
      recommendations.push('Fix failing tests before reorganization to establish clean baseline');
      warnings.push(...baselineReport.summary.failedCommands.map(cmd => `Failing command: ${cmd}`));
    }

    // File organization recommendations
    const relocatableConfigs = Array.from(dependencyMap.configFiles.values())
      .filter(file => file.canRelocate);
    
    if (relocatableConfigs.length > 0) {
      recommendations.push(`${relocatableConfigs.length} configuration files can be safely relocated`);
    }

    const relocatableDocs = Array.from(dependencyMap.documentationFiles.values())
      .filter(file => file.canRelocate);
    
    if (relocatableDocs.length > 0) {
      recommendations.push(`${relocatableDocs.length} documentation files can be moved to docs/ directory`);
    }

    // Dependency warnings
    for (const [, fileRef] of dependencyMap.configFiles) {
      if (fileRef.referencedBy.length > 5) {
        warnings.push(`File ${fileRef.filePath} is heavily referenced (${fileRef.referencedBy.length} files) - move carefully`);
      }
    }
  ...
  private printSummary(plan: ReorganizationPlan): void {
    ...
    console.log('\n📊 File Analysis:');
    console.log(`  • Configuration files: ${plan.dependencyMap.configFiles.size}`);
    console.log(`  • Source files: ${plan.dependencyMap.sourceFiles.size}`);
    console.log(`  • Documentation files: ${plan.dependencyMap.documentationFiles.size}`);
    console.log(`  • Asset files: ${plan.dependencyMap.assetFiles.size}`);

    console.log('\n🔧 Validation Results:');
    console.log(`  • Build system: ${plan.validationResults.buildSystemValid ? '✅' : '❌'}`);
    console.log(`  • Tools available: ${plan.validationResults.toolsAvailable ? '✅' : '❌'}`);
    console.log(`  • Baseline tests: ${plan.baselineReport.summary.allPassed ? '✅' : '❌'}`);
  ...
  public validatePlan(plan: ReorganizationPlan): {
    ...
    if (!plan.baselineReport.summary.allPassed) {
      console.log('⚠️  Warning: Some baseline tests are failing. Reorganization will proceed but may not improve the situation.');
    }
