...
export class DependencyMapper {
  ...
  private async analyzeFile(
    ...
    const canRelocate = !this.criticalRootFiles.has(
      path.basename(relativePath),
    );
    const reason = canRelocate
      ? undefined
      : "Critical file that must remain in root";
  ...
  private extractReferences(content: string, currentFile: string): string[] {
    ...
    const scriptPatterns = [/['"`]([^'"`]*\.(?:js|ts|json|md))['"`]/g];
  ...
  private categorizeFile(relativePath: string, fileRef: FileReference): void {
    ...
    if (
      this.configPatterns.some((pattern) => pattern.test(fileName)) ||
      this.criticalRootFiles.has(fileName)
    ) {
      this.dependencyMap.configFiles.set(relativePath, fileRef);
    } else if (
      this.documentationPatterns.some((pattern) => pattern.test(fileName))
    ) {
  ...
  private async analyzeCrossReferences(): Promise<void> {
    ...
    // Collect all files
    Array.from(this.dependencyMap.configFiles.entries()).forEach(
      ([path, ref]) => {
        allFiles.set(path, ref);
      },
    );
    Array.from(this.dependencyMap.sourceFiles.entries()).forEach(
      ([path, ref]) => {
        allFiles.set(path, ref);
      },
    );
    Array.from(this.dependencyMap.documentationFiles.entries()).forEach(
      ([path, ref]) => {
        allFiles.set(path, ref);
      },
    );
    Array.from(this.dependencyMap.assetFiles.entries()).forEach(
      ([path, ref]) => {
        allFiles.set(path, ref);
      },
    );
  ...
  private findFileReference(filePath: string): FileReference | undefined {
    return (
      this.dependencyMap.configFiles.get(filePath) ||
