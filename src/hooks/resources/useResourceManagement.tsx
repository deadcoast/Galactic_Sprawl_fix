...
import { ResourceType, ResourceState, ResourceThreshold } from '../../types/resources/ResourceTypes';
...
import { ResourceIntegration, createResourceIntegration } from '../../managers/resource/ResourceIntegration';
...
export function useResourceManagement() {
  ...
  const getResourceState = useCallback((type: ResourceType): ResourceState | undefined => {
  ...
  // Get resource amount
  const getResourceAmount = useCallback((type: ResourceType): number => {
  ...
  const hasResources = useCallback((resources: Array<{ type: ResourceType; amount: number }>): boolean => {
    return resources.every(({ type, amount }) => hasResource(type, amount));
  }, [hasResource]);
  
  // Consume a resource
  const consumeResource = useCallback((type: ResourceType, amount: number): boolean => {
  ...
  const consumeResources = useCallback((resources: Array<{ type: ResourceType; amount: number }>): boolean => {
  ...
  const addResources = useCallback((resources: Array<{ type: ResourceType; amount: number }>): void => {
    resources.forEach(({ type, amount }) => {
      resourceManager.addResource(type, amount);
    });
  }, []);
  ...
  const getProductionRate = useCallback((type: ResourceType): number => {
    return resourceStates.get(type)?.production || 0;
  }, [resourceStates]);
  
  // Get resource consumption rate
  const getConsumptionRate = useCallback((type: ResourceType): number => {
  ...
  const getResourceCapacity = useCallback((type: ResourceType): number => {
    return resourceStates.get(type)?.max || 0;
  }, [resourceStates]);
  
  // Get resource percentage
  const getResourcePercentage = useCallback((type: ResourceType): number => {
    const state = resourceStates.get(type);
