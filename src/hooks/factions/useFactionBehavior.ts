...
import { SHIP_STATS as BASE_SHIP_STATS } from '../config/factionShipStats';
import type { ShipStats as ShipStatsType } from '../factionTypes/ship';
...
type FactionStateType = 
  // Space Rats States
  | 'patrolling' | 'pursuing' | 'attacking' | 'aggressive' | 'retreating'
  // Lost Nova States
  | 'hiding' | 'preparing' | 'ambushing' | 'retaliating' | 'withdrawing'
  // Equator Horizon States
  | 'dormant' | 'awakening' | 'enforcing' | 'overwhelming' | 'withdrawing';

type FactionEvent = 
  | 'DETECT_TARGET' | 'TAKE_DAMAGE' | 'ENGAGE_RANGE' | 'LOSE_TARGET'
  | 'TARGET_DESTROYED' | 'HEAVY_DAMAGE' | 'SAFE_DISTANCE' | 'REINFORCEMENTS_ARRIVED'
  | 'DETECT_OPPORTUNITY' | 'PROVOKED' | 'AMBUSH_READY' | 'DETECTED'
  | 'AMBUSH_SUCCESS' | 'AMBUSH_FAILED' | 'THREAT_ELIMINATED' | 'OVERWHELMING_FORCE'
  | 'POWER_THRESHOLD_EXCEEDED' | 'BALANCE_DISRUPTED' | 'FLEET_READY'
  | 'THREAT_DISAPPEARED' | 'BALANCE_RESTORED' | 'RESISTANCE_ENCOUNTERED'
  | 'DOMINANCE_ACHIEVED' | 'OBJECTIVE_COMPLETE' | 'WITHDRAWAL_COMPLETE'
  | 'NO_TARGETS';
...
const STATE_TRANSITIONS: Record<FactionId, StateMachineTransition[]> = {
  'space-rats': [
    ...
    { currentState: 'patrolling', event: 'TAKE_DAMAGE', nextState: 'aggressive' },
    ...
    { currentState: 'attacking', event: 'TARGET_DESTROYED', nextState: 'patrolling' },
    ...
    { currentState: 'aggressive', event: 'NO_TARGETS', nextState: 'patrolling' },
    ...
    { currentState: 'retreating', event: 'REINFORCEMENTS_ARRIVED', nextState: 'aggressive' }
  ],
  'lost-nova': [
    { currentState: 'hiding', event: 'DETECT_OPPORTUNITY', nextState: 'preparing' },
    { currentState: 'hiding', event: 'PROVOKED', nextState: 'retaliating' },
    { currentState: 'preparing', event: 'AMBUSH_READY', nextState: 'ambushing' },
    { currentState: 'preparing', event: 'DETECTED', nextState: 'hiding' },
    { currentState: 'ambushing', event: 'AMBUSH_SUCCESS', nextState: 'hiding' },
    { currentState: 'ambushing', event: 'AMBUSH_FAILED', nextState: 'withdrawing' },
    { currentState: 'ambushing', event: 'HEAVY_DAMAGE', nextState: 'withdrawing' },
    { currentState: 'retaliating', event: 'THREAT_ELIMINATED', nextState: 'hiding' },
    { currentState: 'retaliating', event: 'OVERWHELMING_FORCE', nextState: 'withdrawing' },
    { currentState: 'withdrawing', event: 'SAFE_DISTANCE', nextState: 'hiding' }
  ],
  'equator-horizon': [
    { currentState: 'dormant', event: 'POWER_THRESHOLD_EXCEEDED', nextState: 'awakening' },
    { currentState: 'dormant', event: 'BALANCE_DISRUPTED', nextState: 'awakening' },
    { currentState: 'awakening', event: 'FLEET_READY', nextState: 'enforcing' },
    { currentState: 'awakening', event: 'THREAT_DISAPPEARED', nextState: 'dormant' },
    { currentState: 'enforcing', event: 'BALANCE_RESTORED', nextState: 'withdrawing' },
    { currentState: 'enforcing', event: 'RESISTANCE_ENCOUNTERED', nextState: 'overwhelming' },
    { currentState: 'overwhelming', event: 'DOMINANCE_ACHIEVED', nextState: 'enforcing' },
    { currentState: 'overwhelming', event: 'OBJECTIVE_COMPLETE', nextState: 'withdrawing' },
    { currentState: 'withdrawing', event: 'WITHDRAWAL_COMPLETE', nextState: 'dormant' },
    { currentState: 'withdrawing', event: 'BALANCE_DISRUPTED', nextState: 'enforcing' }
  ]
};
...
function handleStateMachineTransition(
  ...
  const transition = transitions.find(t => 
    t.currentState === currentState && t.event === event
  );
...
export function useFactionBehavior(factionId: FactionId) {
  const [behavior, setBehavior] = useState<FactionBehaviorState>({
    id: factionId,
    name: factionId.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' '),
    fleets: [],
    territory: {
      center: { x: 0, y: 0 },
      radius: 100,
  ...
  useEffect(() => {
    const updateInterval = setInterval(() => {
      ...
      const factionUnits = Array.from(combatManager.getUnitsInRange({ x: 0, y: 0 }, 2000))
      ...
      const updatedTerritory = calculateTerritory(factionUnits, behavior.territory);
    ...
    if (nearbyUnits.length >= 3) {
      const fleet: FactionFleet = {
        ships: nearbyUnits.map(u => {
          ...
          const stats = BASE_SHIP_STATS[shipClass];
          return {
            id: u.id,
            class: shipClass,
...
function calculateRelationships(
  ...
  Object.keys(updatedRelationships).forEach(otherId => {
    ...
    updatedRelationships[otherId as FactionId] = Math.max(-1, Math.min(1, updatedRelationships[otherId as FactionId]));
  });
...
function determineShipClass(unit: CombatUnit): ShipClass {
  ...
  const factionShips = Object.keys(SHIP_STATS).filter(shipClass => {
    if (faction === 'space-rats') {
      return shipClass.includes('rat') || shipClass.includes('marauder') || shipClass.includes('scourge');
    }
    if (faction === 'lost-nova') {
      return shipClass.includes('nova') || shipClass.includes('void') || shipClass.includes('scythe');
    }
    if (faction === 'equator-horizon') {
      return shipClass.includes('celestial') || shipClass.includes('horizon') || shipClass.includes('arbiter');
    }
    return false;
  }) as ShipClass[];
  ...
  if (status.includes('stealth')) {
    return factionShips.find(s => (SHIP_STATS[s] || DEFAULT_SHIP_STATS).abilities.some(a => a.effect.type === 'stealth')) || factionShips[0];
  }
...
function calculateFleetStrength(units: CombatUnit[]): number {
  return units.reduce((total, unit) => {
    ...
    const weaponStrength = stats.weapons.reduce((sum, w) => sum + w.damage * w.accuracy, 0);
    ...
    return total + (baseStrength + weaponStrength + abilityStrength) * healthPercent;
  }, 0);
}
...
function determineNextAction(tactic: FactionBehaviorState['behaviorState']['currentTactic']): string {
  switch (tactic) {
    case 'raid': return 'prepare_raid_fleet';
    case 'defend': return 'fortify_positions';
    case 'expand': return 'scout_territory';
    case 'trade': return 'establish_trade_route';
...
function findNearbyEnemies(state: FactionBehaviorState): CombatUnit[] {
  return Array.from(combatManager.getUnitsInRange(state.territory.center, state.territory.radius))
...
function isAmbushOpportunity(state: FactionBehaviorState): boolean {
  ...
  const stealthShips = state.fleets
    ...
    .filter(ship => ship.class === 'quantum-pariah' || ship.class === 'rogue-nebula').length;
...
function shouldSpawnNewShip(state: FactionBehaviorState, config: typeof FACTION_CONFIGS[FactionId]): boolean {
...
function executePursuit(state: FactionBehaviorState): void {
  state.fleets.forEach(fleet => {
    ...
    if (leadShip?.target) {
      const targetUnit = Array.from(combatManager.getUnitsInRange(leadShip.position, 1000))
...
function executeAmbush(state: FactionBehaviorState): void {
  state.fleets.forEach(fleet => {
    ...
    if (nearbyTargets.length > 0) {
      fleet.ships.forEach(ship => {
        const target = nearbyTargets[Math.floor(Math.random() * nearbyTargets.length)];
        combatManager.engageTarget(ship.id, target.id);
      });
    }
  });
...
function executeBalanceEnforcement(state: FactionBehaviorState): void {
  state.fleets.forEach(fleet => {
    ...
    if (dominantFaction) {
      const targets = Array.from(combatManager.getUnitsInRange(fleet.ships[0].position, 1000))
...
function generatePatrolPoints(territory: FactionTerritory): Array<{ x: number; y: number }> {
...
function findStealthPoints(territory: FactionTerritory): Array<{ x: number; y: number }> {
...
function calculateAmbushPoints(territory: FactionTerritory): Array<{ x: number; y: number }> {
...
function findRetreatPoints(territory: FactionTerritory): Array<{ x: number; y: number }> {
...
function findDominantFaction(state: FactionBehaviorState): string | undefined {
  ...
  Array.from(combatManager.getUnitsInRange(state.territory.center, state.territory.radius))
    .forEach(unit => {
...
function findHighValueTarget(state: FactionBehaviorState): CombatUnit | undefined {
  const targets = Array.from(combatManager.getUnitsInRange(state.territory.center, state.territory.radius))
    .filter(unit => unit.faction !== state.id)
...
function selectSpawnPoint(territory: FactionTerritory): { x: number; y: number } {
