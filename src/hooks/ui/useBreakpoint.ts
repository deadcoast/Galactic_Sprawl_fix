...
export function useBreakpoint(): UseBreakpointResult {
  ...
  const breakpointValues = useMemo(() => {
    return BREAKPOINT_KEYS.reduce<Record<Breakpoint, number>>(
      (acc, key) => {
...
/**
 * @param {Theme} theme - The theme object containing breakpoint definitions.
 * @returns {boolean} - True if the current breakpoint is less than or equal to the target.
 */
const isDown = (breakpoint: Breakpoint, currentBreakpoint: Breakpoint): boolean => {
