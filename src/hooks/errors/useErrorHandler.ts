 ...
 * ```
 */
export function useErrorHandler<E extends Error = Error>(
  ...
  const logError = useCallback(
    (err: E, additionalMetadata: Record<string, unknown> = {}) => {
      // Create an Error object if not already one
      const errorObj = err instanceof Error ? err : new Error(String(err));

      // Log to error service
      errorLoggingService.logError(errorObj, errorType, errorSeverity, {
        componentName,
        ...metadata,
        ...additionalMetadata,
      });
