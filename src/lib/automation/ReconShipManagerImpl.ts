...
export class ReconShipManagerImpl extends EventEmitter {
  ...
  private isNearThreat(ship: ReconShip): boolean {
    ...
    const threats = combatManager.getThreatsInRange(
      ship.position,
      ship.sensors.range
    );
    ...
    const threatLevel = threats.reduce((total, threat) => {
      ...
      const severityFactor = threat.severity === "high" ? 1 : 
                            threat.severity === "medium" ? 0.6 : 0.3;
      
      // Apply sensor accuracy to detection reliability
      const detectionChance = ship.sensors.accuracy * 
        (ship.techBonuses?.detectionRange || 1);
    ...
    const stealthFactor = ship.stealth.level / 100 * 
      (ship.techBonuses?.stealthEfficiency || 1);
