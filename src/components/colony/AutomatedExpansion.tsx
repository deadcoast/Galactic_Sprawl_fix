import { Rocket, Users, ArrowRight, Settings, AlertTriangle, Zap, Database, Star } from 'lucide-react';
import { useState, useEffect, useMemo } from 'react';
import { useVPR } from '../../hooks/useVPR';
import { useGlobalEvents } from '../../hooks/useGlobalEvents';
import type { EventPayload } from '../../hooks/useGlobalEvents';
  ...
  useEffect(() => {
    const updateInterval = setInterval(() => {
      ...
      expansions.forEach(expansion => {
        const isActive = expansion.status !== 'complete' && 
          ...
          return (
            <div
              ...
              className={`p-4 bg-gray-700/50 rounded-lg transition-all duration-500 ${
                activeAnimations[expansion.id] ? `shadow-lg shadow-indigo-500/10 ${animations.active}` : ''
              }`}
            >
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="text-gray-300">
                  {Math.max(0, Math.ceil((expansion.estimatedTime - 
                    (Date.now() - (expansion.lastUpdate || Date.now()))) / 60000))}m
                </span>
              </div>
              ...
              {showParticles && (
                <div className="absolute inset-0 overflow-hidden rounded-lg pointer-events-none">
                  {Array.from({ length: expansion.vprData?.particleSystem?.density || 5 }).map((_, i) => (
                    <div
                      key={i}
              ...
              {techLevel > 1 && (
                <div className="mt-4 flex items-center space-x-4 text-xs text-gray-400">
                  ...
                  <div className="flex items-center">
                    ...
                    <span>+{Math.round((techBonuses.resourceEfficiency - 1) * 100)}% Efficiency</span>
                  </div>
                  <div className="flex items-center">
        ...
        {techLevel >= 2 && (
          <div className="mt-4 p-3 bg-indigo-900/20 border border-indigo-700/30 rounded-lg">
            ...
            <p className="text-xs text-indigo-200">
              Enhanced expansion capabilities unlocked through technology advancement.
              Current tech level provides {Math.round((techBonuses.expansionSpeed - 1) * 100)}% faster expansion.
            </p>
          </div>
        )}
      </div>
