...
import { ExplorationStatus, Sector } from '../../../types/exploration/ExplorationTypes';
...
interface ReconShip {
  id: string;
  name: string;
  type: 'AC27G' | 'PathFinder' | 'VoidSeeker' | 'recon' | 'mining' | 'combat';
...
export const ReconShipCoordination: React.FC<ReconShipCoordinationProps> = ({
  ...
  const renderTaskSharingControls = () => {
    if (activeTab !== 'coordination' || !selectedShipIds.length) {
      return null;
    }
