...
import { Crown, Flag, ChevronRight, Search, Layers, Database, Star, Rocket, Zap, AlertTriangle, Bell, ZoomIn, ZoomOut, HelpCircle, Map } from 'lucide-react';
...
export function SprawlView({ empire }: { empire: EmpireDetails }) {
  ...
  const [assets, setAssets] = useState<Asset[]>([
    ...
    },
    {
      ...
      resources: ['Helium-3'],
      tradeConnections: [
        { target: 'alpha-colony', volume: 0.8 }
      ],
      status: 'active',
    ...
    },
    {
      ...
      population: 2000,
      tradeConnections: [
        { target: 'gamma-sphere', volume: 0.3 }
      ],
      status: 'active',
        ...
        return currentAssets.map(asset => {
                  if (asset.status === 'active' && Math.random() > 0.8) {
                    ...
                    if (populationChange > 0) {
                      addNotification('success', `${asset.name}: Population increased by ${populationChange}`, asset.id);
                    }
  ...
  const addNotification = (type: 'info' | 'warning' | 'success', message: string, assetId: string) => {
    const notification = {
      id: `${assetId}-${Date.now()}`,
      type,
      message,
  ...
  const getAssetIcon = (type: Asset['type']) => {
    switch (type) {
      case 'colony': return Crown;
      case 'dysonSphere': return Star;
      case 'starStation': return Rocket;
      case 'tradeHub': return Database;
  ...
  const getAssetColor = (asset: Asset) => {
    ...
    switch (asset.type) {
      case 'colony': return 'cyan';
      case 'dysonSphere': return 'orange';
      case 'starStation': return 'purple';
      case 'tradeHub': return 'emerald';
      default: return 'blue';
    }
  };
  ...
  const filteredAssets = assets.filter(asset => 
    view.searchQuery
      ? asset.name.toLowerCase().includes(view.searchQuery.toLowerCase())
      : true
  );
  ...
  return (
    <GameLayout empireName={empire.name} bannerColor={empire.bannerColor}>
      <div className="absolute inset-0 bg-gray-900 overflow-hidden">
        ...
        >
          <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1506318137071-a8e063b4bec0?q=80&w=3000')] bg-cover opacity-20 transform scale-100" style={{ transition: 'transform 120s linear' }} />
          <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1419242902214-272b3f66ee7a?q=80&w=2013')] bg-cover opacity-10 transform scale-150" style={{ transition: 'transform 180s linear' }} />
        ...
        <div className="absolute top-6 left-6 right-6 flex items-center justify-between z-10">
          ...
          <div className="flex space-x-2">
            ...
            <button 
              className="p-2 bg-gray-800/90 hover:bg-gray-700/90 rounded-lg backdrop-blur-sm transition-colors group relative"
            >
              <HelpCircle className="w-5 h-5 text-cyan-400" />
              <div className="absolute right-0 top-full mt-2 w-64 p-3 bg-gray-800/95 rounded-lg border border-gray-700 hidden group-hover:block">
                <h4 className="text-white font-medium mb-2">Navigation Help</h4>
                <ul className="space-y-1 text-sm text-gray-300">
                  <li className="flex items-center"><ZoomIn className="w-4 h-4 mr-2" /> Zoom in view</li>
                  <li className="flex items-center"><ZoomOut className="w-4 h-4 mr-2" /> Zoom out view</li>
                  <li className="flex items-center"><Flag className="w-4 h-4 mr-2" /> Mark territory</li>
                  <li className="flex items-center"><ChevronRight className="w-4 h-4 mr-2" /> Navigate sectors</li>
                </ul>
              </div>
            </button>
          </div>
            ...
            {filteredAssets.map((asset) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <div 
                      className={`w-24 h-24 rounded-full bg-${color}-500/20 animate-pulse relative`}
                      style={{
                        boxShadow: `0 0 ${20 + asset.developmentLevel * 30}px ${asset.developmentLevel * 20}px rgba(${
                          ...
                          color === 'emerald' ? '16, 185, 129' : '59, 130, 246'
                        }, ${0.1 + asset.developmentLevel * 0.2})`
                      }}
                    >
                      <div 
                        className={`w-16 h-16 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-${color}-400/30`}
                        style={{
                          transform: `translate(-50%, -50%) scale(${0.8 + Math.sin(Date.now() / 1000) * 0.1})`
                        }}
                      >
                        <div className={`w-8 h-8 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-${color}-300/40 flex items-center justify-center animate-spin-slow`}>
                          <Icon className={`w-5 h-5 text-${color}-200`} />
                        </div>
                      </div>
                      
                      {/* Development Level Rings */}
                      {Array.from({ length: Math.ceil(asset.developmentLevel * 5) }).map((_, i) => (
                        <div
                          key={i}
                    ...
                    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 text-center">
                      ...
                      {asset.status === 'constructing' && (
                        <div className="text-yellow-300/70 text-sm flex items-center justify-center">
                    ...
                    {/* Hover Tooltip */}
                    <div className="absolute left-1/2 bottom-full -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                        ...
                        {asset.resources && view.activeOverlays.includes('resources') && (
                          <div className="flex items-center gap-2 text-sm">
                            {asset.resources.map(resource => (
                              <span key={resource} className="text-gray-300">{resource}</span>
                            ))}
                          </div>
                        )}
                      </div>
        ...
        <div className="fixed bottom-6 left-6 space-y-2">
          {view.notifications.map(notification => (
            <div
              key={notification.id}
              className={`px-4 py-2 rounded-lg backdrop-blur-sm flex items-center space-x-2 transition-all ${
                notification.type === 'success' ? 'bg-green-900/80 text-green-200' :
                notification.type === 'warning' ? 'bg-yellow-900/80 text-yellow-200' :
            ...
            >
              {notification.type === 'success' && <Bell className="w-4 h-4" />}
              {notification.type === 'warning' && <AlertTriangle className="w-4 h-4" />}
              {notification.type === 'info' && <Database className="w-4 h-4" />}
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-indigo-100 font-medium">{empire.name} Map</span>
            </div>
