...
// Map click handler
export type MapClickHandler = (x: number, y: number, viewport: MapViewport) => void;
...
export const BaseMap: React.FC<BaseMapProps> = ({
    ...
    if (selection.length > 0) {
      ...
      for (const selected of selection) {
        if (selected.selected) {
          const [screenX, screenY] = worldToScreen(selected.coordinates.x, selected.coordinates.y);
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      // Check if clicked on an entity from the selection
      const clickedEntity = selection.find(entity => {
        const [entityX, entityY] = worldToScreen(entity.coordinates.x, entity.coordinates.y);
        const dx = x - entityX;
        const dy = y - entityY;
        const distance = Math.sqrt(dx * dx + dy * dy);
        return distance <= 20 * viewport.scale; // Using same radius as selection circle
      });

      // Call onEntityClick if an entity was clicked
      if (clickedEntity && onEntityClick) {
        onEntityClick(clickedEntity.entityId, clickedEntity.entityType);
        return;
      }
    ...
    [isPanning, screenToWorld, worldToScreen, selection, viewport.scale, onEntityClick, onMapClick]
  );
