...
export function BaseDataTable<T>({
  ...
  const handleSortChange = useCallback(
    (columnId: string) => {
      ...
      setSortDirection(prev => {
        const newDirection = columnId === sortColumn ? (prev === 'asc' ? 'desc' : 'asc') : 'asc';
  ...
  const renderRows = () => {
    ...
    const visibleData = virtualized ? sortedData.slice(startIndex, endIndex + 1) : sortedData;
  ...
  return (
    <div className={cn('overflow-hidden rounded-md bg-white', className)}>
      <div style={virtualContainerStyle} onScroll={virtualized ? handleScroll : undefined}>
