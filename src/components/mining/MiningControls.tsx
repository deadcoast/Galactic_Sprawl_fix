...
import { Settings, ChevronRight, Database, Pickaxe, Star } from 'lucide-react';
...
export function MiningControls({ resource, techBonuses, onExperienceGained }: MiningControlsProps) {
    ...
    const miningInterval = setInterval(() => {
      ...
      setMiningProgress(prev => {
        ...
        if (newProgress >= 100) {
          ...
          experience.totalXP = experience.baseAmount * 
  ...
  }, [autoMine, resource, effectiveExtractionRate, effectiveEfficiency, onExperienceGained, techBonuses]);
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            <div className={`p-2 rounded-lg ${
              resource.type === 'mineral' ? 'bg-cyan-500/20' :
              resource.type === 'gas' ? 'bg-purple-500/20' :
              'bg-amber-500/20'
            }`}>
              {resource.type === 'mineral' ? <Pickaxe className="w-5 h-5 text-cyan-400" /> :
               resource.type === 'gas' ? <Database className="w-5 h-5 text-purple-400" /> :
               <Star className="w-5 h-5 text-amber-400" />}
            </div>
            <div>
              <h3 className="font-medium text-white">{resource.name}</h3>
              <div className="text-sm text-gray-400">
                {resource.type.charAt(0).toUpperCase() + resource.type.slice(1)} • {resource.distance}ly
              </div>
            </div>
          </div>
          ...
          <div className="h-2 bg-gray-700 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all ${
                resource.type === 'mineral' ? 'bg-cyan-500' :
                resource.type === 'gas' ? 'bg-purple-500' :
                'bg-amber-500'
              }`}
        ...
        <div className="mt-4 space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="p-2 bg-gray-800 rounded">
              ...
              <div className="text-sm text-green-400">+{Math.round((techBonuses.extractionRate - 1) * 100)}%</div>
            </div>
            <div className="p-2 bg-gray-800 rounded">
              <div className="text-xs text-gray-400">Efficiency</div>
              <div className="text-sm text-green-400">+{Math.round((techBonuses.efficiency - 1) * 100)}%</div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-gray-800/50 rounded-lg p-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div className="text-lg font-medium text-white">{Math.round(effectiveEfficiency * 100)}%</div>
          </div>
        </div>
      </div>
    </div>
