    ...
    >
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-lg font-medium">
          {title}
          {componentName && (
            <span className="ml-1 text-sm font-normal opacity-80">({componentName})</span>
          )}
        </h3>
      </div>
        ...
        {showErrorDetails && (
          <details className="mt-2 rounded border border-red-200 bg-red-100 p-2 text-xs dark:border-red-800 dark:bg-red-900/50">
            ...
            <div className="mt-1 overflow-auto p-1">
              ...
              <p>{error.message}</p>
              {error.stack && (
                <pre className="mt-2 max-h-32 overflow-auto text-xs whitespace-pre-wrap opacity-80">
                  {error.stack}
                </pre>
              )}
            </div>
      ...
      <div className="mt-4 flex gap-2">
        {showResetButton && (
          <button
            onClick={handleReset}
            className="rounded-md bg-red-100 px-3 py-1.5 text-sm font-medium text-red-800 hover:bg-red-200 focus:ring-2 focus:ring-red-500 focus:ring-offset-2 focus:outline-none dark:bg-red-800 dark:text-red-200 dark:hover:bg-red-700"
        ...
        {actionText && onAction && (
          <button
            ...
            className="rounded-md bg-white px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-100 focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 focus:outline-none dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700"
          >
            {actionText}
          </button>
        )}
