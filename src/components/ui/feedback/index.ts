...
export const Alert: React.FC<AlertProps> = ({
  ...
  const variantStyles: Record<string, React.CSSProperties> = {
    ...
    outlined: { background: 'transparent', border: `1px solid ${color.main}`, color: color.main },
    standard: { background: color.bg, borderLeft: `4px solid ${color.main}`, color: '#fff' },
  };
...
export const Progress: React.FC<ProgressProps> = ({
  ...
  const bufferPercentage = bufferValue ? Math.min(100, Math.max(0, (bufferValue / max) * 100)) : 0;
  ...
  if (variant === 'indeterminate') {
    return React.createElement('div', { style: { width: '100%' } },
      label && React.createElement('div', {
        ...
        React.createElement('span', { style: { fontSize: '12px', color: '#888' } }, label)
      ),
      React.createElement('div', {
        style: {
          width: '100%',
  ...
  return React.createElement('div', { style: { width: '100%' } },
    ...
    React.createElement('div', {
      ...
      variant === 'buffer' && bufferValue && React.createElement('div', {
        style: {
          position: 'absolute',
...
export const Toast: React.FC<ToastProps> = ({
  ...
  return React.createElement('div', {
    style: {
      ...
      animation: exiting ? 'slideOut 0.2s ease-out forwards' : 'slideIn 0.3s ease-out',
      zIndex: 9999,
      maxWidth: '400px',
    },
    role: 'alert',
    ...
    React.createElement('span', { style: { flex: 1, fontSize: '14px' } }, message),
...
const toastListeners: Set<(toast: ToastProps & { id: string }) => void> = new Set();
