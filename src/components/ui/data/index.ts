...
export function DataTable<T extends Record<string, unknown>>({
  ...
  return React.createElement('div', {
    style: { width: '100%', overflow: 'auto', borderRadius: '8px', background: '#1a1a2e' },
  },
          ...
          sortedData.map((row, rowIndex) =>
            React.createElement('tr', {
              ...
              style: {
                ...
                background: selectedRow === rowIndex
                  ? 'rgba(74, 158, 255, 0.2)'
                  : hoveredRow === rowIndex
        ...
        React.createElement('div', null,
          React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '8px' } },
            ...
            React.createElement('h3', {
              style: { margin: 0, color: '#fff', fontSize: isCompact ? '13px' : '15px', fontWeight: 600 },
            }, title)
          ),
          description && React.createElement('p', {
            style: { margin: '4px 0 0 18px', color: '#888', fontSize: '12px' },
          }, description)
        )
      ),
      React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '8px' } },
...
export const Metric: React.FC<MetricProps> = ({
  ...
  return React.createElement('div', {
    ...
    React.createElement('div', {
      ...
      React.createElement('span', {
        style: { color: '#888', fontSize: sizeConfig.label, textTransform: 'uppercase', letterSpacing: '0.5px' },
      }, label)
    ),
    React.createElement('div', {
      ...
      React.createElement('span', {
        style: { color: '#fff', fontSize: sizeConfig.value, fontWeight: 700, fontFeatureSettings: '"tnum"' },
      }, formattedValue),
    ...
    trend && React.createElement('div', {
      ...
      trendLabel && React.createElement('span', {
        style: { color: '#666' },
      }, trendLabel)
    )
  );
  ...
  return React.createElement('div', {
    ...
    sortedItems.map((item, index) => {
      ...
      return React.createElement('div', {
        ...
        React.createElement('div', {
          style: { display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: '24px' },
        },
          React.createElement('div', {
            style: {
              ...
              color: '#fff',
              transition: 'all 0.2s',
              boxShadow: item.status === 'active' ? `0 0 8px ${config.color}` : 'none',
            },
          }, !isCompact && (item.icon || config.icon)),
          showConnector && !isLast && React.createElement('div', {
            style: {
              width: '2px',
              flexGrow: 1,
        ...
        React.createElement('div', {
          style: {
            ...
            background: isHovered && item.onClick ? 'rgba(255, 255, 255, 0.03)' : 'transparent',
            borderRadius: '4px',
          ...
          item.description && React.createElement('div', {
            style: { color: '#888', fontSize: isCompact ? '12px' : '14px', lineHeight: 1.5 },
          }, item.description),
          isDetailed && item.metadata && React.createElement('div', {
            style: {
              display: 'flex',
              flexWrap: 'wrap',
              gap: '12px',
            ...
            Object.entries(item.metadata).map(([key, val]) =>
              React.createElement('div', {
                ...
                React.createElement('span', { style: { color: '#666' } }, `${key}:`),
                React.createElement('span', { style: { color: '#fff' } }, String(val))
              )
            )
          )
        )
