 ...
 */
export const ResourceFlowDiagram = memo(
  function ResourceFlowDiagram({
    nodes,
    connections,
    width = 800,
    height = 600,
    ...
    const networkNodes = useMemo(() => {
      return nodes.map(node => {
        ...
        const size = node.capacity
          ? Math.max(30, Math.min(60, 30 + (node.capacity / 100) * 30))
          : 40;
    ...
    const networkEdges = useMemo(() => {
      return connections.map(connection => {
        ...
        const label =
          showResourceIcons && connection.resourceTypes.length > 0
  ...
  createPropsComparison<ResourceFlowDiagramProps>({
    ...
    propsAreEqual: (prevProps, nextProps) => {
      ...
      if (
        Array.isArray(prevProps.nodes) &&
        Array.isArray(nextProps.nodes) &&
      ...
      if (
        Array.isArray(prevProps.connections) &&
        Array.isArray(nextProps.connections) &&
  ...
  return (
    <div className="resource-flow-node-details">
      <div className="resource-flow-node-details-header">
        <h3>{node.name}</h3>
        <button className="resource-flow-node-details-close" onClick={onClose}>
          ×
        </button>
      </div>
