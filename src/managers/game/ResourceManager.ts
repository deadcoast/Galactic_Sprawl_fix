...
export class ResourceManager extends AbstractBaseManager<ResourceManagerEvent> {
   ...
   * Ensures the amount stays within min/max bounds
   */
  setResourceAmount(type: ResourceType, amount: number): void {
    ...
    if (state) {
      ...
      // Ensure amount is clamped between min and max
      const newAmount = Math.max(state.min, Math.min(state.max, amount));
      ...
      this.logResourceError(`set-${resourceType}`, {
        code: 'INVALID_RESOURCE',
        message: `Resource ${resourceType} not found.`,
      });
    }
  }
  ...
  setStorageEfficiency(level: keyof typeof STORAGE_EFFICIENCY): void {
    ...
    if (this.config.defaultResourceLimits) {
      // Iterate through ResourceType enum values
      for (const resourceType of Object.values(ResourceType).filter(
        ...
        if (limits && typeof limits.max === 'number') {
          ...
          if (state) {
            ...
            // Use console.info or a dedicated logger instead of warn if this is expected behavior
            errorLoggingService.logInfo(
              `[ResourceManager] Updated ${resourceType} storage capacity: ${oldMax.toFixed(2)} -> ${state.max.toFixed(2)}`,
              {
                service: 'ResourceManager',
                method: 'setStorageEfficiency',
                resourceType: resourceType,
              }
            );
          }
  ...
  private setResourceThreshold(
    ...
    const state = this.resources.get(resourceType);
    if (state) {
      const oldValue = state[thresholdType];
      if (oldValue !== value) {
        state[thresholdType] = value;
      ...
      this.logResourceError(`set-threshold-${resourceType}`, {
        code: 'INVALID_RESOURCE',
        message: `Resource ${resourceType} not found when setting threshold.`,
      });
    }
  }
    ...
    this.optimizationStrategies.set('optimize-transfers', {
      ...
      apply: () => {
        ...
        for (const flow of flowValues) {
          ...
          // Check if unknown source is below threshold
