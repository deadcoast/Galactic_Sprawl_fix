...
def parse_arguments():
    ...
    parser.add_argument('max_depth', nargs='?', type=int, default=None,
    ...
    parser.add_argument('--use-ignore', action='store_true',
    ...
    parser.add_argument('--add-ignore', nargs='+', default=[],
                        help='Add patterns to the default ignore list for this run only')
    parser.add_argument('-c', '--custom-dir', type=str, default=None,
...
def generate_tree(directory, prefix='', max_depth=None, current_depth=0, exclude_patterns=None):
    """
    ...
    for i, item in enumerate(items):
        ...
        if item.is_dir():
            ...
            # For files, just add them to the tree
...
def main():
    ...
    exclude_patterns = list(args.exclude)  # Convert to list to avoid modifying the original
    ...
    if args.custom_dir:
        ...
        if not target_dir.exists() or not target_dir.is_dir():
            print(f"Error: The specified directory '{args.custom_dir}' does not exist or is not a directory.")
            return
    else:
