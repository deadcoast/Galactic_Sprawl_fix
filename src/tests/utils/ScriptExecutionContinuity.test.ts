...
export class ScriptExecutionValidator {
   ...
   */
  static parseScriptReferences(scripts: PackageJsonScript): Map<string, string[]> {
  ...
  static validateScriptAfterMove(
    ...
    for (const [oldPath, newPath] of fileMoves.entries()) {
      if (originalScript.includes(oldPath)) {
        updatedScript = updatedScript.replace(new RegExp(oldPath, 'g'), newPath);
      }
    }
  ...
  static updateScriptsForReorganization(
    ...
    for (const [scriptName, command] of Object.entries(originalScripts)) {
      ...
      if (!validation.isValid) {
        warnings.push(`Script '${scriptName}' may have issues: ${validation.errors.join(', ')}`);
      }
    }
  ...
  describe('Property 3: Script Execution Continuity', () => {
    ...
    it('should preserve script execution for any valid file reorganization', () => {
      ...
      for (const [scriptName, command] of Object.entries(testScripts)) {
        const validation = ScriptExecutionValidator.validateScriptAfterMove(command, fileMoves);
        ...
        if (command.includes('eslint.config.js')) {
          expect(validation.updatedScript).toContain('config/linting/eslint.config.js');
        }
        if (command.includes('vitest.config.ts')) {
          expect(validation.updatedScript).toContain('config/testing/vitest.config.ts');
        }
    ...
    it('should correctly identify file references in script commands', () => {
      ...
      for (const testCase of testCases) {
        ...
        for (const expectedRef of testCase.expectedRefs) {
          const found = extractedRefs.some(ref => 
            ref === expectedRef || 
    ...
    it('should handle complex script reorganization scenarios', () => {
      ...
      const complexScripts: PackageJsonScript = {
        'build:prod': 'tsc --project tsconfig.build.json && vite build --config vite.config.prod.ts',
        'test:coverage': 'vitest --config vitest.config.ts --coverage --reporter=json --outputFile=coverage/report.json',
        'lint:fix': 'eslint src/ --config eslint.config.js --fix && prettier --write src/**/*.ts --config .prettierrc',
        'deploy': 'npm run build:prod && node scripts/deploy.js --config deploy.config.json'
      };
      ...
      expect(Object.keys(result.updatedScripts)).toHaveLength(Object.keys(complexScripts).length);
      
      // Property assertion: Updated scripts should contain new paths
      expect(result.updatedScripts['build:prod']).toContain('config/typescript/tsconfig.build.json');
      expect(result.updatedScripts['build:prod']).toContain('config/build/vite.config.prod.ts');
      expect(result.updatedScripts['test:coverage']).toContain('config/testing/vitest.config.ts');
      expect(result.updatedScripts['lint:fix']).toContain('config/linting/eslint.config.js');
      expect(result.updatedScripts['lint:fix']).toContain('config/build/.prettierrc');
      expect(result.updatedScripts['deploy']).toContain('config/deployment/deploy.config.json');
      
      // Property assertion: Scripts should remain executable (no excessive path nesting)
      for (const [scriptName, updatedScript] of Object.entries(result.updatedScripts)) {
        const validation = ScriptExecutionValidator.validateScriptAfterMove(updatedScript, new Map());
        expect(validation.isValid).toBe(true);
      }
    });
    ...
    it('should preserve script safety after reorganization', () => {
      ...
      for (const script of safeScripts) {
        ...
        const isSafe = ScriptExecutionValidator.isScriptSafeAfterReorganization(script, movedFiles);
        
        // For this test, we expect most basic scripts to be safe or easily updatable
        if (script.includes('eslint') || script.includes('prettier') || script.includes('vitest')) {
    ...
    it('should handle edge cases in script parsing', () => {
      ...
      const edgeCaseScripts: PackageJsonScript = {
        ...
        'multiple-configs': 'jest --config jest.config.js && eslint --config eslint.config.js'
      };

      for (const [scriptName, command] of Object.entries(edgeCaseScripts)) {
        // Should handle parsing without errors
        const references = ScriptExecutionValidator.parseScriptReferences({ [scriptName]: command });
        ...
        const validation = ScriptExecutionValidator.validateScriptAfterMove(command, new Map());
    ...
    it('should maintain script command structure after updates', () => {
      ...
      const originalScript = 'eslint src/ --config eslint.config.js --fix && prettier --write src/**/*.ts --config .prettierrc';
      const fileMoves = new Map([
        ['eslint.config.js', 'config/linting/eslint.config.js'],
        ['.prettierrc', 'config/build/.prettierrc']
      ]);

      const validation = ScriptExecutionValidator.validateScriptAfterMove(originalScript, fileMoves);
