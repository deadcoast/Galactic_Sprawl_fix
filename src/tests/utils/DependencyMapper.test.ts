...
class MockDependencyMapper {
  ...
  categorizeFile(fileName: string): 'critical' | 'config' | 'documentation' | 'source' | 'asset' {
  ...
  describe('Property 1: Build System Integrity', () => {
    ...
    it('should preserve build system integrity for any valid file structure', () => {
      ...
      for (const files of testStructures) {
        ...
        const criticalFiles = ['package.json', 'vite.config.ts', 'tsconfig.json', 'eslint.config.js'];
        ...
        const nonCriticalFiles = files.filter(f => !criticalFiles.includes(f));
    ...
    it('should correctly identify relocatable vs non-relocatable files', () => {
      ...
      for (const testCase of testCases) {
        expect(mockMapper.canRelocate(testCase.fileName)).toBe(testCase.expectedRelocatable);
        expect(mockMapper.categorizeFile(testCase.fileName)).toBe(testCase.expectedCategory);
      }
    });
    ...
    it('should handle edge cases without breaking', () => {
      ...
      const edgeCases = [
        ...
        ['package.json', 'vite.config.ts', 'tsconfig.json', 'README.md', 'CHANGELOG.md'],
        // Project with mixed file types
        ['package.json', 'vite.config.ts', 'tsconfig.json', '.prettierrc', 'src/main.ts', 'ERRORS.json']
      ];
      ...
      for (const files of edgeCases) {
        ...
        for (const file of files) {
          ...
          expect(['critical', 'config', 'documentation', 'source', 'asset']).toContain(category);
        }
      }
    });
  });
