...
describe('EventCommunication', () => {
  ...
  describe('initialization', () => {
    it('should subscribe to system message events', () => {
      ...
      expect(moduleEventBus.subscribe).toHaveBeenCalledWith('SYSTEM_MESSAGE_ACK', expect.any(Function));
    });
  ...
  describe('observables', () => {
    ...
    it('should provide an observable of message payloads', () => {
      ...
      const subscription = resourceSystem.getMessagePayloads<{ test: string }>('payload-message').subscribe({
        next: nextFn
      });
