  ...
  describe('Property 2: Dependency Resolution Preservation', () => {
    ...
    it('should preserve path resolution for any valid file move', () => {
      ...
      for (const testCase of testCases) {
        ...
        const normalizedExpected = testCase.expectedResolution.replace(/\\/g, '/');
        const normalizedActual = updatedImport.replace(/\\/g, '/');
        
        // Remove file extensions for comparison
        const withoutExt = (p: string) => p.replace(/\.(ts|js|tsx|jsx)$/, '');
        expect(withoutExt(normalizedActual)).toBe(withoutExt(normalizedExpected));
      }
    });
    ...
    it('should handle complex directory restructuring scenarios', () => {
      ...
      const scenario = {
        // Original structure
        originalFiles: {
          'src/utils/helper.ts': ['src/components/App.tsx', 'src/services/api.ts'],
          'src/components/Button.tsx': ['src/components/App.tsx'],
          'src/services/api.ts': ['src/components/App.tsx', 'src/utils/helper.ts']
        },
      ...
      for (const [originalFile, referencingFiles] of Object.entries(scenario.originalFiles)) {
        const newPath = scenario.newPaths[originalFile];
        
        for (const referencingFile of referencingFiles) {
          // Calculate original import
          const originalImport = PathResolver.resolveRelativePath(referencingFile, originalFile);
    ...
    it('should handle basic bidirectional references', () => {
      ...
      const aImportsB = PathResolver.resolveRelativePath(deps.fileA, deps.fileB);
