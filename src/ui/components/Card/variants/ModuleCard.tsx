    ...
    const moduleHeader = (
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          <div className={cn('mr-2 h-3 w-3 rounded-full', statusColors[module.status])} />
          <span className="font-medium">{statusLabels[module.status]}</span>
        </div>
        {module.timeRemaining && module.timeRemaining > 0 && (
          <div className="text-sm text-gray-600">{formatTime(module.timeRemaining)}</div>
        )}
      </div>
    );

    // Create the footer with action buttons
    const moduleFooter = (showUpgrade || showRepair) && (
          ...
          <div>
            ...
            <div className="h-2 overflow-hidden rounded-full bg-gray-200">
              <div
                className={cn(
                  'h-full rounded-full',
                  module.durability > 60
                    ? 'bg-green-500'
                    : module.durability > 30
                      ? 'bg-yellow-500'
                      : 'bg-red-500'
                )}
        ...
        <div className="mt-3 border-t border-gray-200 pt-2 text-sm">
          {hasConsumption && (
            <div>
              ...
              <div className="ml-2 grid grid-cols-2 gap-x-2">
