---
inclusion: always
description: This document provides implementation examples of Event Handling patterns as used in the Galactic Sprawl codebase. These patterns ensure type-safe event communication between different systems.
globs: **/*.tsx, **/*.ts, src/components/**/*.tsx", "src/hooks/use*Events.ts
---
  ...
  private setupEventSubscriptions(): void {
    ...
    this.unsubscribeHandlers.push(
      moduleEventBus.subscribeToMany(
        [EventType.RESOURCE_PRODUCED, EventType.RESOURCE_CONSUMED, EventType.RESOURCE_TRANSFERRED],
...
export function useResourceEvents(
  ...
  useEffect(() => {
    ...
    const unsubscribe = moduleEventBus.subscribeToMany(resourceEventTypes, (event: BaseEvent) => {
      if (!isResourceEvent(event)) return;
