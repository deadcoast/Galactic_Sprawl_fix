...
export function useDebugOverlay() {
  ...
  const [networkStats, setNetworkStats] = useState<{latency: number; drops: number}>({ latency: 0, drops: 0 });
  const [gcStats, setGCStats] = useState<{time: number; count: number}>({ time: 0, count: 0 });
  ...
  const collectSystemMetrics = useCallback((): {
    ...
    const frameDrops = window.performance.getEntriesByType('frame').filter(entry => entry.duration > 16.67).length;
    const gcTime = window.performance.getEntriesByType('gc').reduce((total, entry) => total + entry.duration, 0);
  ...
  const collectCombatMetrics = useCallback((unit: Unit): CombatStats => {
    const weaponEffects = unit.weapons.map(weapon => ({
      ...
      accuracy: weapon.stats ? weapon.stats.hits ? weapon.stats.hits / (weapon.stats.hits + weapon.stats.misses) : 0 : 0,
  ...
  useEffect(() => {
    const updateInterval = setInterval(() => {
      ...
      units.forEach((combatUnit) => {
        ...
        if (unitStatus) {
          const threatLevel = factionState?.relationshipWithPlayer !== undefined 
            ? factionState.relationshipWithPlayer < 0 ? 1 : 0
            : 0;
          ...
          states[unit.id] = {
            aiState: {
              ...
              cooldowns: unit.weapons.reduce((acc, w) => ({
                ...acc,
                [w.id]: w.cooldown
              }), {})
            },
