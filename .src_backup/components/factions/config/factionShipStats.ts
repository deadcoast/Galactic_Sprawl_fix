import { ShipClass, ShipStats, ShipDisplayStats } from '../factionTypes/ship';
...
export function getShipDisplayStats(stats: ShipStats): ShipDisplayStats {
  return {
    weapons: {
      ...
      accuracy: stats.weapons.reduce((sum, w) => sum + w.accuracy, 0) / stats.weapons.length
    },
    defense: {
      hull: stats.health,
      shield: stats.shield,
    ...
    systems: {
      ...
      radar: Math.round(stats.weapons.reduce((sum, w) => sum + w.range, 0) / 100),
      efficiency: Math.round((1 - stats.weapons.reduce((sum, w) => sum + w.cooldown, 0) / 10) * 100)
    }
  };
} 