import { Sword, Shield, Rocket, AlertTriangle } from 'lucide-react';

interface EquatorHorizonShipProps {
  id: string;
  name: string;
  type: 'celestialArbiter' | 'etherealGalleon' | 'stellarEquinox' | 'chronosSentinel' | 'nebulasJudgement' | 'aetherialHorizon' | 'cosmicCrusader' | 'balancekeepersWrath' | 'eclipticWatcher' | 'harmonysVanguard';
...
export function EquatorHorizonShip({ 
  ...
  const getShipRole = (shipType: string) => {
    switch (shipType) {
      case 'celestialArbiter': return 'Ultimate Balance Enforcer';
      case 'etherealGalleon': return 'Ancient Energy Vessel';
      case 'stellarEquinox': return 'Harmony Cruiser';
      case 'chronosSentinel': return 'Time Manipulation Ship';
      case 'nebulasJudgement': return 'Swift Justice Vessel';
      case 'aetherialHorizon': return 'First Contact Flagship';
      case 'cosmicCrusader': return 'Threat Neutralizer';
      case 'balancekeepersWrath': return 'Order Dreadnought';
      case 'eclipticWatcher': return 'Stealth Observer';
      case 'harmonysVanguard': return 'Order Spearhead';
      default: return 'Ancient Vessel';
    }
  };
    ...
    <div className="bg-amber-900/20 border border-amber-700/30 rounded-lg p-6">
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <div className={`px-3 py-1 rounded-full text-sm ${
          status === 'engaging' ? 'bg-amber-900/50 text-amber-400' :
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className={`px-2 py-1 rounded text-xs ${
            ...
            tactics === 'defensive' ? 'bg-amber-900/50 text-amber-400' :
      ...
      {status === 'disabled' && (
        <div className="mt-4 p-3 bg-red-900/20 border border-red-700/30 rounded-lg flex items-start space-x-2">
          ...
          <span className="text-sm text-red-200">Ancient systems critically damaged</span>
        </div>
      )}
    </div>
  );
