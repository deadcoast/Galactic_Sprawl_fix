...
import { Users, Star, X, AlertTriangle, ChevronRight, Filter, Search } from 'lucide-react';
...
export function HiringPanel({ tier, onHire, onClose }: HiringPanelProps) {
  ...
  const filteredRoles = roles.filter(role => {
    if (filter !== 'all' && role.origin !== filter) return false;
    if (searchQuery && !role.name.toLowerCase().includes(searchQuery.toLowerCase())) return false;
    return true;
  });
  ...
  return (
    ...
    >
      ...
      >
              ...
              >
                ...
                {role.origin !== 'native' && (
                  <div className={`mt-2 px-2 py-1 rounded-full text-xs inline-block ${
                    role.origin === 'refugee' ? 'bg-blue-900/50 text-blue-400' : 'bg-red-900/50 text-red-400'
                  }`}>
        ...
        {/* Warnings */}
        <AnimatePresence>
          {selectedRole && (filteredRoles.find(r => r.id === selectedRole)?.requirements?.tier ?? 0) > tier && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
