...
export function CapitalShipEffect({ type, status, shieldStrength, weaponCharge, quality }: CapitalShipEffectProps) {
  const getShipColor = () => {
    switch (type) {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      ...
      {type === 'harbringerGalleon' && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className={`w-3/4 h-3/4 rounded-full border-2 border-${color}-500/30 animate-spin-slow`} />
        </div>
      )}
