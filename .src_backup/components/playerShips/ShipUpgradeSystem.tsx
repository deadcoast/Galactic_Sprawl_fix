import { Shield, Crosshair, Rocket, ArrowRight, Lock, AlertTriangle } from 'lucide-react';
...
export function ShipUpgradeSystem({ ship, onUpgrade, onPreviewUpgrade }: ShipUpgradeSystemProps) {
    ...
    <div className={`bg-${color}-900/20 border border-${color}-700/30 rounded-lg p-6`}>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <div className="text-sm text-gray-400">Tier {ship.tier} → {ship.tier + 1}</div>
        </div>
          ...
          {Object.entries(ship.stats).map(([stat, values]) => (
            <div key={stat} className="p-3 bg-gray-700/50 rounded-lg">
              ...
              <div className="flex items-center justify-between">
                <div className="text-white">{values.current}{values.unit}</div>
                <div className="flex items-center text-sm">
                  <ArrowRight className="w-4 h-4 text-indigo-400 mx-2" />
                  <span className="text-indigo-400">
                    {values.upgraded}{values.unit} ({getStatDifference(values.current, values.upgraded)})
                  </span>
                </div>
              </div>
            </div>
