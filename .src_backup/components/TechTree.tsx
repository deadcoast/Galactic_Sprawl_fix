...
import { Zap, Shield, Radar, Rocket, Database, Lock, Check, Crosshair, Ship, Sword, Users } from 'lucide-react';
...
interface TechNode {
  ...
  requirements: string[];
  unlocked: boolean;
  category: 'infrastructure' | 'warFleet' | 'reconFleet' | 'miningFleet' | 'weapons' | 'defense' | 'special' | 'synergy';
}
...
const techNodes: TechNode[] = [
  ...
  },
  {
    ...
    description: 'Unlocks additional modules including trading post and improved population management',
    tier: 2,
    icon: 'database',
  ...
  // Weapons - Tier 3
  {
    id: 'maximum-damage',
    name: 'Maximum Damage',
    description: 'Unlock Spark Rounds, Big Bang Rockets, Recirculating Gauss, and Maurader Rail Gun',
    tier: 3,
    icon: 'crosshair',
          ...
          return (
            ...
            >
              ...
              >
                ...
                {!node.unlocked && <Lock className="w-4 h-4 absolute top-1 right-1 text-gray-300/70" />}
                {node.unlocked && <Check className="w-4 h-4 absolute top-1 right-1 text-green-300" />}
              </button>
          ...
          <div className="flex items-start justify-between">
            ...
            {nodeIcons[selectedNode.icon] && (
              <div className={`p-3 ${getNodeColor(selectedNode)} rounded-lg`}>
                {React.createElement(nodeIcons[selectedNode.icon], { className: 'w-6 h-6 text-white' })}
              </div>
            )}
          </div>
        </div>
