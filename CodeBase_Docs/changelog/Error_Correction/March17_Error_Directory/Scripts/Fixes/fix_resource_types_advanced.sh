...
# Function to fix variable declarations
fix_variable_declarations() {
  ...
  # Fix variable declarations with resource-related names
  local temp_file=$(mktemp)
  sed -E "s/(const|let|var)\s+(resource|resourceType|type)\s*:\s*string\b/\1 \2: ResourceType/g" "$file" > "$temp_file"
  mv "$temp_file" "$file"
  
  # Fix variable initializations
  temp_file=$(mktemp)
  sed -E "s/(const|let|var)\s+([a-zA-Z0-9_]+)\s*:\s*string\s*=\s*['\"](minerals|gas|energy|food|water)['\"];/\1 \2: ResourceType = ResourceType.\U\3\E;/g" "$file" > "$temp_file"
  mv "$temp_file" "$file"
  
  # Fix resource map types
  temp_file=$(mktemp)
  sed -E "s/:\s*{\s*\[key:\s*string\]:\s*string\s*}/: { [key: string]: ResourceType }/g" "$file" > "$temp_file"
  mv "$temp_file" "$file"
}
